...
| `DATA_PATH`                                                | `utils/gamedata/shared.js`、`updateGameData.ts`                                                      | `.env` + `.env.production`，均为绝对路径 | 核实于 2026-08-11：prod/dev 三份 `.env` 的值统一为 `/arkrog/shared/ArknightsGameData`。**2026-08-03 迁移前** `.env` 曾写 `~` 开头、靠 `.env.production` 的绝对路径覆盖才可用（Node 不展开 `~`），该层叠依赖已消失                                                                                                                                |
...
| `OCR_ENDPOINT_HOST`                                        | `routers/mapRecognition.ts`（默认 `ocr.tencentcloudapi.com`）                                        | dev + **prod** `.env` 均已配（prod 于 2026-08-16） | 生产应设 `ocr.internal.tencentcloudapi.com`（内网端点，解析到 169.254.1.10，不占公网带宽）。**该域名只在腾讯云 VPC 内解析**，本地开发/CI 必须留默认公网域名。换端点由代码走 TC3 签名的 `host` 参数完成，不是只改 URL |
| `OCR_REGION`                                               | `routers/mapRecognition.ts`（默认 `ap-guangzhou`）                                                   | dev + **prod** `.env` 均已配（prod 于 2026-08-16） | 服务器在 ap-beijing-3，prod 已设 `ap-beijing`；**region 必须与内网端点所在地域一致**，两者要一起改                                                                                                                                                                           |
...
| `OCR_LOCAL_ENDPOINT` / `OCR_LOCAL_TOKEN` / `OCR_LOCAL_FIRST` / `OCR_LOCAL_TIMEOUT_MS` | `utils/localOcr.ts`、`routers/mapRecognition.ts` | dev + prod 均已配（2026-08-18） | 自建本地 OCR 服务（`https://ocr.arkrec.com/ocr`）。**`OCR_LOCAL_FIRST=1` 即本地为链首主力、云端退为兜底；删掉这一行并重启即整体回滚到纯云端**，不必改代码或重新部署。主力超时默认 5000ms，比影子角色短（影子超时只丢一条观测，主力超时是用户纯白等）。旧名 `OCR_SHADOW_ENDPOINT`/`OCR_SHADOW_TOKEN` 仍被兼容读取。影子对照另由 `OCR_SHADOW=1` 单独开关，**本地已是链首后不要开**（会对同一张图打两遍本地） |
...
上游数据仓库（核实于 2026-08-11）：`DATA_PATH` 指向 **`/arkrog/shared/ArknightsGameData`**，**prod 与 dev 共用这一份**（旧路径 `/home/ubuntu/ArknightsGameData` 随 2026-08-03 迁移作废）。它是**浅克隆**（`git rev-parse --is-shallow-repository` = true，按 `--depth=1` 方式维护），没有完整历史，不要在服务器上对它做依赖历史的 git 操作。remote 指向上游 `Kengxxiao/ArknightsGameData`。因为两环境共用，拉取它会同时影响 prod 与 dev——拉取与重建的完整口径见[数据管线](data-pipeline.md) 第 3.5 节。
