...
外加全局月度硬上限 `OCR_MONTHLY_QUOTA`（默认 **10000**）。

> 这个上限**不是免费额度护栏,是失控消费的兜底闸门**。腾讯云通用印刷体识别免费额度只有 1000 次/月，项目已明确接受超出后按量付费（**0.15 元/次**，1 万次以内档），所以上限定在远高于免费额度处，只为拦住被刷爆的情况。触及上限后接口对所有人返回 429，直到自然月翻页（按 **UTC** 月份计，即北京时间每月 1 号 08:00 重置）。
>
> 改这个值要清楚对应费用：上限 10000 意味着最坏情况单月账单约 `(10000 − 1000) × 0.15 ≈ 1350 元`。
>
> 计数键 `rl:ocr:quota:<年月>` 存在 Redis，且**已在 `app.ts` 的启动清缓存里豁免**——否则每次部署都会把本月已用量抹平、闸门永不触发（曾发生过）。prod 与 dev 用同一个腾讯云账号但 `REDIS_PREFIX` 不同，两边计数各算各的；dev 用量很小，有意忽略这个偏差。
