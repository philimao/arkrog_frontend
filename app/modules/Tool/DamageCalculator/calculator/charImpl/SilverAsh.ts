...
/** 银灰伤害计算器 */
export function SilverAsh(input: CalculatorInput): CalculatorOutput {
    // 干员养成加成
    ...
    const atkSpeedBuff = context.in_game_buff_add.attack_speed.calculate() + context.relic_rune_add.attack_speed.calculate(); // 额外攻击速度
    ...
    switch (skillKey) {
        ...
        case "skchr_svrash_3": {
            ...
            const skillDph = ((atk + atkBuffInAdd) * (1 + skillBuffIn + atkBuffInMul) + atkBuffInAdd) * atkBuffFinalMul * 1.15;//默认攻击精英/领袖
            const skillDamage = Math.max(skillDph - enemyDef, skillDph * 0.05) * damage_scale * damage_scale_phy;
            const skillDamageMag = Math.max(skillDph * 0.1 * (1 - enemyMagRes * 0.01) / 1.15) * damage_scale * damage_scale_mag;
...
// 注册银灰伤害计算器
registerCalculatorImpl("SilverAsh", SilverAsh);
