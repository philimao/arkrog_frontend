...
| 仓库根同时存在 `yarn.lock` 与 `pnpm-lock.yaml` | 仓库根 yarn.lock、pnpm-lock.yaml | 已确认（2026-06-11 核实并存） | 不同开发者/CI 可能用不同包管理器解析出不同依赖树 | 待决策（确定包管理器后删除另一个锁文件；工程约定见 [../../../../../CONTRIBUTING.md](../../../../../CONTRIBUTING.md) 与 [../../../../../docs/testing.md](../../../../../docs/testing.md)） |
