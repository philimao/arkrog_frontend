...
export default function TournamentSchedule({
  ...
  const renderScheduleUnit = (schedule: Map<string, TournamentGame>, date: string, session?: string, sessionIndex?: number) => {
      ...
      <div key={index} className="mb-4">
        ...
        <table className="w-full border-collapse table-fixed">
          <tbody className="divide-y divide-mid-gray">
            {!!sessions.size && <tr className="bg-black-gray text-center divide-x divide-mid-gray">
            ...
            {dates.map((date, dateIndex) => (
              <tr key={dateIndex} className="bg-black-gray-70 divide-x divide-mid-gray ">
                ...
                {!!sessions.size
                  ? Array.from(sessions)?.map((session, sessionIndex) => (
                    renderScheduleUnit(schedule, date.toDateString(), session, sessionIndex)
                  ))
