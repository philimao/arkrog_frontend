...
| 改上游同步自动化（`arkrog_backend/util-scripts/gamedata-sync.sh`、其 cron 调度），或改主题白名单 `SUPPORTED_ROGUE_TOPICS`（`arkrog_backend/utils/gamedata/shared.js`）                                                                                                                                  | 更新 [docs/gamedata-auto-sync-plan.md](docs/gamedata-auto-sync-plan.md)；若改的是运维口径（命令、频率、两遍执行），同步 [docs/data-pipeline.md](docs/data-pipeline.md) 第 3.5 节。**白名单只增不减**——删条目会让该主题从生产库消失，且新主题必须前端适配先部署再翻开关                                                                          |
...
| 改 `app/modules/Tool/BlackFlowMap/ScreenshotRecognizer.tsx` 或 `index.tsx` 中识别结果的**呈现与落盘行为**（自动填入的门槛、候选展示条件、覆盖/合并、`detectedNodesByMap` 两层模型）                                                                                                                     | 更新[黑流模块 01](app/modules/Tool/BlackFlowMap/docs/01-screenshot-recognition.md) 第四节。**这块最容易漏**：算法没动、只动 UI 时不会触发上一条，但第四节描述的正是这里的行为                                                                                                                                                                      |
