        ...
        <div className="col-span-2">
          {ongoingTournaments.map((tournament, index) => (
            ...
            >
              ...
              <div className="w-full max-w-40"><div className="w-full aspect-square bg-light-gray rounded-xl" /></div>
              <div className="flex flex-col pl-4 gap-1">
                <div className="text-3xl font-bold">{tournament.name}{tournament.season && `#${tournament.season}`}</div>
                ...
                {<div>版本：{currentTopic.name}{tournament.edition}</div>}
  ...
  const renderTournaments = () => {
    ...
    return <>
      {editions.map((edition) => (
        <div className="mb-16 last-of-type:mb-0">
          ...
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-3 lg:grid-cols-4 grow">
            {tournamentsByEdition.get(edition)?.map((tournament) => (
              <div className="flex flex-col items-center">
                {/* TODO: replace with images from backend */}
                <div role="button" className="w-full aspect-square bg-light-gray mb-2 rounded-xl relative" onClick={() => {
                  navigate(tournament.id)
                }}>
                  {ongoingTournaments.includes(tournament) && <div className="absolute bg-ak-dark-red top-6 -right-2 px-2 rounded-sm">进行中</div>}
                </div>
