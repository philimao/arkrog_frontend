...
| `yarn docs:gen` | 从源码重新生成模块 `docs/generated/` 三份清单文档 |
...
`generated/` 下三份文档是 `yarn docs:gen` 的产物，**禁止手改**——手改会在下次生成时被覆盖，且生成物与源码的一致性是 CI 校验目标。
...
- [ ] 若改了 utils.ts 名单 / blackboard.ts 注册 / charImpl：已重跑 `yarn docs:gen` 并提交 generated/ 变更
...
| `docs/` | 全仓库索引（`README.md`）、术语表（`glossary.md`）、跨模块/跨仓库流程（`data-pipeline.md`）、仓库级工程说明（`testing.md`） |
| 模块根 `README.md`、`calculator/charImpl/README.md`、`test/DamageCalculator/README.md` | 3–5 行指针文件，只放链接，防止双源漂移 |
