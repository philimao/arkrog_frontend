...
| `OCR_SHADOW_ENDPOINT` / `OCR_SHADOW_TOKEN` / `OCR_SHADOW_TIMEOUT_MS` | `utils/localOcr.ts` | 灰度期 prod 配置 | 自建本地 OCR 服务（`https://ocr.arkrec.com/ocr`）。**不配置则整块静默禁用**，链路行为与接入前完全一致。超时默认 6000ms（2026-08-16 按生产实测分布从 4000 上调：本地推理 P99.5 3.6s + 排队最长 1.9s + 跨区网络 0.3s）—— 影子与云端并发发出，只在比云端慢时才影响总耗时 |
