...
| `OCR_SHADOW_ENDPOINT` / `OCR_SHADOW_TOKEN` / `OCR_SHADOW_TIMEOUT_MS` | `utils/localOcr.ts` | 灰度期 prod 配置 | 自建本地 OCR 服务（`https://ocr.arkrec.com/ocr`）。**不配置则整块静默禁用**，链路行为与接入前完全一致。超时默认 4000ms —— 影子与云端并发发出，只在比云端慢时才影响总耗时 |
