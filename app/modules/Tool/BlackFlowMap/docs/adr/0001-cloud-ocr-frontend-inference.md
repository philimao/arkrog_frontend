...
2. **tesseract worker 会崩掉整个进程**。同一个 worker 连续处理若干张图后必然抛 wasm 内存越界，且该异常在 worker 线程内抛出、`try/catch` 接不住，直接杀进程。而复用 worker 正是长驻服务的常规做法——除了「一图一进程」没有干净解法。
...
**付出**：识别按次计费，因此**限流从优化项变成功能的一部分**；多了一个外部依赖，腾讯云 OCR 不可用时功能整体不可用（返回 502）。

本条的成本模型已被 [ADR-0002](0002-multi-account-multi-action-ocr-chain.md) 取代：改用多接口降级链后不再需要自设的月度硬上限。
