...
| # | 校验 | 失败提示 | 备注 |
|---|---|---|---|
| ① | `URLValidation(data.url)` 归一化 | （见第 5 节，**警告不阻断**） | 返回值仅在为空串时才拦截，实际几乎必然放行 |
...
通过后：剥除各成员 `charData` → 组装 `data.team`/`data.stageId` → **残留的 `console.log(data)` 打印全量表单**（第 7 节）→ `_post("/record/submit", data)` → 成功则 `setRecords(响应)` 并关 Modal → `setTimeout(2000)` 后 `fetchStagePreview(true)`（时序契约见模块 02 第 10 节）。

> ⚠️ `handleSubmit` 没有 try/catch：服务端拒绝（链接格式、解析失败的中文提示、权限不足等，见模块 02 第 4.2 节）会变成 unhandled rejection——**无 toast、Modal 不关、表单不复位**。登记见 [known-issues.md](known-issues.md)。
...
3. **b23.tv 短链**：提取 `https://b23.tv/...` 片段，`fetch("/api/parse-redirect?url=" + 短链)`，**以响应 body 文本直接作为新 URL**——该端点是幻影端点（第 6 节），且这里不检查 `res.ok`。
...
**"警告不阻断"是真实行为**：第 14 步的两个失败分支 toast 之后**仍然返回当前 URL 字符串**，`handleSubmit` 第①步的 `if (validatedURL)` 对非空字符串恒为真——提交照常发出，真正的拦截发生在后端 `isValidRecordUrl`（仅本地 HEAD 有，见模块 02）。前端警告只是提示，不是校验。
...
现状故障链：线上 404 → `resRaw.text()` 拿到 404 响应体 → 被当作 URL 继续归一化 → 终检 toast 警告但不阻断 → 后端 `isValidRecordUrl` 以 400 拒绝（本地 HEAD）→ 前端无 try/catch，用户看到的是"点了提交没反应"。
...
3. 前端必须补 `res.ok` 校验，当前实现会把任何错误响应体当作解析结果。
...
- **残留 `console.log(data)`**：提交前把含队伍明细的完整表单打到控制台；另有两段注释掉的调试 `useEffect`。
- **无 try/catch 的提交**：服务端错误提示全部不可达（第 4 节）。
