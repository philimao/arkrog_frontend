...
费用口径（2026-08-10 确认）：通用印刷体识别免费额度 1000 次/月，超出后 **0.15 元/次**（1 万次以内档，1 万~10 万次档为 0.10 元/次）。项目**明确接受超出免费额度按量付费**，因此全局月度上限 `OCR_MONTHLY_QUOTA` 定在 10000（对应最坏单月账单约 1350 元），它的职责是拦住被刷爆的失控消费，而不是把用量压在免费额度内。
